...
## 1.5 CI/CD and Deployment

### Task 1.5.1: Set Up Build Pipeline
**Owner:** Backend Engineer  
**Estimated:** 2 hours  
...
**Description:**
Create build scripts and systemd service definitions.

**DoD:**
- [ ] Makefile targets: `build`, `test`, `lint`, `clean`
- [ ] systemd service files for each binary
- [ ] Build script: `/opt/app/scripts/build.sh`
- [ ] Deploy script: `/opt/app/scripts/deploy.sh`
...
BUILD_DIR := ./bin
CMDS := api ws price resolver archiver
...
3. Database connections can be established from a test program
4. Redis is accepting connections with proper persistence
5. Nginx serves static files and proxies to upstream (even if upstream doesn't exist yet)
